function applyTheme(){
  document.body.classList.toggle("dark-mode",localStorage.getItem("sortitDarkMode")==="1");
}
const API="/api";
...
async function updateNavUser(){
  ...
  const user=await getMe().catch(()=>null);
  if(user){
    b.textContent=user.name;b.title=user.isAdmin?"Administrator account":"Logged in";b.classList.add("user-name");
    if(user.isAdmin && !document.getElementById("adminLink")){
      const nav=document.getElementById("mainNav");
      if(nav) nav.insertAdjacentHTML("beforeend",'<a class="nav-link" id="adminLink" href="admin.html">Admin</a>');
    }
...
async function setupSettings(){
  ...
  if(dark) dark.checked=localStorage.getItem("sortitDarkMode")==="1";
  document.getElementById("saveSettings")?.addEventListener("click",()=>{
    localStorage.setItem("sortitNotify",document.getElementById("notifyIssues").checked?"1":"0");
    localStorage.setItem("sortitRemember",document.getElementById("rememberSettings").checked?"1":"0");
    const dark=document.getElementById("darkMode");
    if(dark)localStorage.setItem("sortitDarkMode",dark.checked?"1":"0");
    applyTheme();toast("Settings saved.");
  });
...
applyTheme();setupNav();setupHome();setupReport();setupSettings();
