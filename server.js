...
function setSession(res,token){ res.setHeader("Set-Cookie",`sortit_session=${encodeURIComponent(token)}; HttpOnly; SameSite=Lax; Path=/; Max-Age=2592000`); }
...
app.post("/api/auth/register",async(req,res)=>{
  try{
    ...
    res.status(201).json({user:db.getUserById(result.lastInsertRowid)});
